...
def run_evaluations(args, logger):
    ...
    for info in dataset.infos:
        ...
        # ============== LOP DEFENSE ==============
        n_preds_before = preds_cp4f['boxes_lidar'].shape[0]
        raw_points = frame['points']  # spoofed input cloud, LiDAR frame

        if preds_cp4f['boxes_lidar'].shape[0] > 0:
            keep_mask = defense.filter(
                points=raw_points,
                pred_boxes=preds_cp4f['boxes_lidar'],
                pred_labels=preds_cp4f['pred_labels'],
            )
            removed_scores = preds_cp4f['score'][~keep_mask]
            if len(removed_scores) > 0:
                print(f"removed {len(removed_scores)} preds, mean score={removed_scores.mean():.3f}")            # Apply the mask to every per-prediction field in the anno dict.
            preds_cp4f['boxes_lidar']  = preds_cp4f['boxes_lidar'][keep_mask]
            preds_cp4f['score']        = preds_cp4f['score'][keep_mask]
            preds_cp4f['pred_labels']  = preds_cp4f['pred_labels'][keep_mask]
            preds_cp4f['name']         = preds_cp4f['name'][keep_mask]
        ...
        n_preds_after = preds_cp4f['boxes_lidar'].shape[0]
