...
def run_evaluations(args, logger):
    ...
    for info in dataset.infos:
        ...
        #use msf preds to get metadata, replace them with actual pred boxes/scores/labels from cp4f preds for evaluation
    ...
    for k in tqdm(range(num_windows), desc="Evaluating windows"):
        ...
        if k >= 1:
            for fid, seg, loc_idx in zip(fids, segs, local_indices):
                ...
                # success = hidden, so record failures-to-detect as ASR numerator
